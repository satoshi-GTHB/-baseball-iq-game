...
function play(id){
  ...
  if(id==="secondary-lead"){
    fieldCard.classList.add("lead-mode");const self=field.querySelector(".self-runner");self.style.left="75%";self.style.top="60%";leadAction.disabled=true;leadPlayStart.disabled=false;leadHelp.textContent="「プレー開始」を押してください。";field.scrollIntoView({behavior:"smooth",block:"center"});return;
  }else{
...
const mainTabs=[...document.querySelectorAll("[data-main]")];mainTabs.forEach(b=>b.addEventListener("click",()=>{mainTabs.forEach(x=>x.setAttribute("aria-pressed",String(x===b)));document.querySelector("#batter").hidden=b.dataset.main!=="batter";document.querySelector("#runner").hidden=b.dataset.main!=="runner";document.querySelector("#batter-demo-slot").append(fieldCard);if(b.dataset.main==="batter"){play("batter-infield");return}const selected=document.querySelector("[data-sub][aria-pressed='true']").dataset.sub;if(selected==="lead"){document.querySelector("#lead-demo-slot").append(fieldCard);play("secondary-lead")}else if(selected==="judge"){document.querySelector("#judge-demo-slot").append(fieldCard);play("judge-first-short")}}));
const subTabs=[...document.querySelectorAll("[data-sub]")];subTabs.forEach(b=>b.addEventListener("click",()=>{subTabs.forEach(x=>x.setAttribute("aria-pressed",String(x===b)));["check","lead","judge"].forEach(id=>document.querySelector("#"+id).hidden=id!==b.dataset.sub);if(b.dataset.sub==="check"){document.querySelector("#batter-demo-slot").append(fieldCard);return}if(b.dataset.sub==="lead"){document.querySelector("#lead-demo-slot").append(fieldCard);play("secondary-lead");return}document.querySelector("#judge-demo-slot").append(fieldCard);play("judge-first-short")}));
