(() => {
  ...
  function resetPlay() {
    state.balls = 0; state.strikes = 0; state.contact = null; state.battedBallLocation = null; state.fielders = []; state.continuationFielders=[];state.continuationReason=null;state.errorFielder=null;state.plateResult = null; state.pitchSequence = [];state.platePitchCount=0;
  ...
  function judge(result) {
    ...
    act(`${label} → ${to === 0 ? "ホーム" : to + "塁"}（${result}）`, () => {
      const reason = state.continuationReason || (state.playMode === "runnerEvent" ? state.eventReason : null),from=runnerFrom(key),pickoff=state.playMode==="runnerEvent"&&state.eventReason==="牽制"&&!state.continuationReason,sameBase=key!=="batter"&&to===from;
      ...
      state.selected = state.continuationReason ? null : state.playMode === "runnerEvent" ? nextEventParticipantKey() : null;
      state.pendingTarget = null;
    });
  }
  ...
  function finishPlay() {
    ...
    const unresolvedRunners=!!state.contact&&state.bases.slice(1).some((runner,index)=>runner&&!decisionFor(`base${index+1}`));
    if(!runnerEvent&&unresolvedRunners){$("#status").textContent="塁上にいたすべての走者の結果を入力してください";return;}
  ...
  function applyHitResult(kind){state.plateResult=kind;state.log.push(kind);}
  ...
  function showPlateErrorChoices(){const popup=$("#errorChoices"),route=[...state.fielders];popup.querySelector("b").textContent="誰のエラー？";const list=popup.querySelector("div");list.innerHTML="";route.forEach((number,index)=>{const button=document.createElement("button"),kind=index===route.length-1?"捕球エラー":"送球エラー";button.type="button";button.textContent=number;button.dataset.errorFielder=number;button.onclick=()=>error(kind,number);list.appendChild(button);});popup.hidden=false;$("#hitChoices").hidden=true;}
  function result(kind) {
    if (state.runnerMode) {const pickoff=state.playMode==="runnerEvent"&&state.eventReason==="牽制",batterSafe=state.playMode==="plate"&&decisionFor("batter")?.result==="SAFE";if(batterSafe&&!state.plateResult&&kind==="安打"){$("#hitChoices").hidden=false;$("#errorChoices").hidden=true;return;}if(batterSafe&&!state.plateResult&&kind==="野選"){act("野選",()=>{state.plateResult="野選";state.log.push("野選");});return;}if(batterSafe&&!state.plateResult&&kind==="エラー"){showPlateErrorChoices();return;}if(kind==="エラー"&&(pickoff||state.playMode==="plate"&&state.decisions.some(d=>d.result==="SAFE"))){$("#errorChoices").hidden=false;$("#hitChoices").hidden=true;}return;}
  ...
  function error(kind,errorFielder=null) {if(state.runnerMode){const pickoff=state.playMode==="runnerEvent"&&state.eventReason==="牽制",batterSafe=state.playMode==="plate"&&decisionFor("batter")?.result==="SAFE",minimum=kind==="送球エラー"?1:2;if(pickoff){if(state.fielders.length<minimum){$("#status").textContent=minimum===1?"先に牽制した守備番号を選択してください":"牽制した守備番号と送球先を選択してください";return;}act(`牽制：${errorAdvanceMark(kind)}`,()=>{state.continuationReason=kind;});return;}if(batterSafe&&!state.plateResult){act(`${errorFielder||""}番のエラー`,()=>{state.errorFielder=errorFielder?String(errorFielder):null;state.plateResult=kind;state.log.push(`${errorFielder||""}番のエラー`);});return;}if(state.playMode!=="plate"||state.fielders.length+(state.continuationFielders?.length||0)<2){$("#status").textContent="先に送球先の守備番号を選択してください";return;}act(`追加進塁：${errorAdvanceMark(kind)}`,()=>{state.continuationReason=kind;state.selected=null;state.pendingTarget=null;});return;}act(kind,()=>{state.plateResult=kind;commitDeferredResult();});}
