(() => {
  ...
  function chatgptPrompt(){const side=$("input[name=orderSide]:checked").value;return `野球のオーダー票を読み取り、元のスコアブックアプリへ戻るまで、利用者を短い日本語で順番に案内してください。\n\nまだ写真が添付されていない場合は、まず「入力欄の＋を押し、カメラを選んでオーダー票を撮影してください。撮影後、そのまま送信してください」とだけ案内し、写真が送られるまで待ってください。\n\n写真が届いたら、先発9名に加え、記載があればベンチ入りメンバーも含め最大20名を読み取ってください。背番号はuniformNumberRaw、先発選手の守備番号（1～9）はdefensiveNumberRawへ入れてください。推測で空欄を埋めず、不明箇所は空文字にしてwarningsへ理由を書いてください。sideは必ず"${side}"にしてください。先発はbattingOrderを1～9、ベンチ入りメンバーは0、rowsは最大20名、confidenceは0～1にしてください。\n\n読み取り後は、次の形のJSONを必ずJSONコードブロック1個で提示してください。\n{"teamNameRaw":"","side":"${side}","rows":[{"battingOrder":1,"playerNameRaw":"","uniformNumberRaw":"","defensiveNumberRaw":"","confidence":0,"warnings":[]},{"battingOrder":0,"playerNameRaw":"","uniformNumberRaw":"","defensiveNumberRaw":"","confidence":0,"warnings":[]}],"overallConfidence":0,"imageWarnings":[]}\n\nJSONコードブロックの直後に「コード右上のコピーを押し、元のスコアブックアプリへ戻って『② 回答を取り込む』を押してください」と案内してください。JSON以外をコードブロック内へ入れないでください。`;}
  ...
  function setup(){
    ...
    orderSteps.innerHTML="<li><b>① ChatGPTを開く</b>を押します（指示文は自動コピーされます）。</li><li>ChatGPTの入力欄で<b>貼り付けて送信</b>します。</li><li>その後は、ChatGPTの案内どおりに撮影・コピーを進めます。</li><li>この画面へ戻り、<b>② 回答を取り込む</b>を押します。</li>";
    orderImage.closest(".file-button").hidden=true;$("#orderPreview").hidden=true;
    copyOrderPrompt.hidden=false;copyOrderPrompt.disabled=false;copyOrderPrompt.textContent="① ChatGPTを開く";copyOrderPrompt.classList.add("next-action");openChatgpt.hidden=true;
