(() => {
  ...
  function parseOrderMatrix(raw){const lines=raw.replace(/^```[^\n]*\n?/i,"").replace(/```\s*$/i,"").split(/\r?\n/).map(line=>line.trim()).filter(Boolean),parsed=lines.map(line=>({line,columns:splitMatrixLine(line)})),teamEntry=parsed.find(item=>String(item.columns[0]).toUpperCase()==="TEAM"),sideEntry=parsed.find(item=>String(item.columns[0]).toUpperCase()==="SIDE"),headerIndex=parsed.findIndex(item=>item.columns[0]==="打順"&&item.columns.includes("選手名")&&item.columns.includes("背番号"));if(!teamEntry||!sideEntry||headerIndex<0)return null;const rows=[];for(const item of parsed.slice(headerIndex+1)){let columns=item.columns;if(columns.length<5){const match=item.line.match(/^([0-9])\s+(.+?)\s+(\d+)\s+([1-9]?)\s+(0(?:\.\d+)?|1(?:\.0+)?)\s*(.*)$/);if(match)columns=match.slice(1);else continue;}const battingOrder=Number(columns[0]);if(!Number.isInteger(battingOrder)||battingOrder<0||battingOrder>9)continue;rows.push({battingOrder,playerNameRaw:(columns[1]||"").trim(),uniformNumberRaw:(columns[2]||"").trim(),defensiveNumberRaw:(columns[3]||"").trim(),confidence:Number(columns[4])||0,warnings:(columns.slice(5).join(" ").trim()?[columns.slice(5).join(" ").trim()]:[])});if(rows.length===20)break;}if(!rows.length)return null;return {teamNameRaw:teamEntry.columns.slice(1).join(" ").trim(),side:sideEntry.columns[1]?.trim(),rows,overallConfidence:rows.reduce((sum,row)=>sum+row.confidence,0)/rows.length,imageWarnings:[]};}
