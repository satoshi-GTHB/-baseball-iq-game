(() => {
  ...
  function chatgptPrompt(){const side=$("input[name=orderSide]:checked").value;return `野球のオーダー票を読み取り、元のスコアブックアプリへ戻るまで、利用者を短い日本語で順番に案内してください。操作案内と読取結果を同じ回答へ混ぜないでください。\n\nまだ写真が添付されていない場合は、次の文章だけを表示し、写真が送られるまで待ってください。\n「入力欄の＋を押し、カメラを選んでオーダー票を撮影してください。撮影後、そのまま写真のみを送信していただければ、アプリに貼り付けるための表を返します。表が表示されたら、その回答をそのままコピーし、元のスコアブックアプリへ戻って『② 回答を取り込む』を押してください。」\n\n写真が届いたら追加の指示を求めず、直ちに読み取ってください。先発9名に加え、記載があればベンチ入りメンバーも含め最大20名を読み取ってください。推測で空欄を埋めず、読めない値は空欄にして警告欄へ理由を書いてください。\n\n写真を受け取った回答はJSONやMarkdown表にせず、必ず次の縦棒マーカー形式だけを返してください。説明、挨拶、注釈、コード枠は付けないでください。改行がコピー時に消えても解析できるよう、各選手行の先頭には必ずROW|を付け、すべての項目を半角の縦棒 | で区切ってください。値の中では縦棒を使わないでください。\nTEAM|読み取ったチーム名\nSIDE|${side}\nCOLUMNS|打順|選手名|背番号|守備番号|確信度|警告\nROW|1|山田 太郎|7|6|0.90|\nROW|0|佐藤 次郎|12||0.70|氏名の一部が不鮮明\n\n先発選手の打順は1～9、控え選手は0にしてください。守備番号は先発だけ1～9、控え選手は空欄にしてください。確信度は0～1の小数にしてください。選手1名につき必ずROWを1個作り、列を増減・並べ替えないでください。写真全体の注意事項は、該当する選手の警告欄へ簡潔に記載してください。TEAM、SIDE、COLUMNS、ROW以外の文字を前後へ追加しないでください。`;}
