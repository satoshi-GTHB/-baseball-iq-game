(() => {
  ...
  async function handlePhoto(file) {
    ...
    try{await navigator.clipboard.writeText(chatgptPrompt());shareButton.disabled=false;shareButton.classList.add("next-action");toast("撮影できました。次は②「ChatGPTへ共有」を押してください");}
  ...
  async function copyPrompt(){try{await navigator.clipboard.writeText(chatgptPrompt());$("#copyPrompt").hidden=true;$("#shareOrderImage").disabled=false;$("#shareOrderImage").classList.add("next-action");toast("コピーしました。次は②「ChatGPTへ共有」を押してください");}catch{toast("コピーできませんでした。ブラウザのクリップボード許可を確認してください");}}
  async function launchChatgptOrderReader(){const opened=window.open("https://chatgpt.com/","_blank","noopener,noreferrer");try{await navigator.clipboard.writeText(chatgptPrompt());toast("ChatGPTを開きました。貼り付け後、カメラで撮影して送信してください");}catch{toast(opened?"ChatGPTを開きました。指示文のコピーを許可してください":"ChatGPTを開けませんでした");}}
  async function importOrderFromClipboard(){try{const text=(await navigator.clipboard.readText()).trim();if(!text)throw new Error("クリップボードが空です");$("#chatgptJson").value=text;importChatgptJson();}catch(error){const field=$("#chatgptJson"),label=field.closest("label");label.hidden=false;field.focus();toast(`自動取得できません：${error.message}。表示された欄へ回答を貼り付けてください`);}}
  async function shareOrderImage(){const file=$("#orderImage").files[0];if(!file)return;try{if(!navigator.share||!navigator.canShare?.({files:[file]}))throw new Error("この端末では写真共有を利用できません");try{await navigator.clipboard.writeText(chatgptPrompt());}catch{}await navigator.share({files:[file],title:"野球オーダー票",text:chatgptPrompt()});$("#shareOrderImage").classList.remove("next-action");const button=$("#importChatgptJson");button.dataset.ready="true";button.classList.add("next-action");updateJsonImportButtons();toast("ChatGPTで指示文を貼り付けて送信し、回答をコピーして戻ってきてください");}catch(error){if(error.name!=="AbortError")toast(`${error.message}。ChatGPTでカメラから写真を添付してください`);}}
  ...
  function setup(){
    ...
    orderSteps.innerHTML="<li><b>① オーダー票を撮影</b>します（指示文は自動コピーされます）。</li><li><b>② ChatGPTへ共有</b>を押し、共有先でChatGPTを選びます。</li><li>ChatGPTの入力欄で<b>貼り付け → 送信</b>します。</li><li>回答の<b>コピー</b>を押し、この画面へ戻ります。</li><li><b>③ 回答を取り込む</b>を押します。</li>";
    orderImage.closest(".file-button").hidden=false;orderImage.closest(".file-button").firstChild.textContent="① オーダー票を撮影";$("#orderPreview").hidden=true;
    copyOrderPrompt.hidden=true;copyOrderPrompt.disabled=true;openChatgpt.hidden=true;
    const shareButton=$("#shareOrderImage");shareButton.hidden=false;shareButton.disabled=true;shareButton.textContent="② ChatGPTへ共有";
    ...
    $("#showChatgptSteps").onclick=()=>{$("#chatgptWorkflow").hidden=false;$("#orderImage").closest(".file-button").classList.add("next-action");toast("まず①「オーダー票を撮影」を押してください");};
    $("#copyPrompt").onclick=copyPrompt;$("#importChatgptJson").onclick=importOrderFromClipboard;
