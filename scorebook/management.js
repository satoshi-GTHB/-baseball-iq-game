(() => {
  ...
  function importChatgptJson(){try{const source=$("#chatgptJson").value.trim();extraction=parseOrderMatrix(source);if(!extraction){let raw=source.replace(/[“”]/g,'"').replace(/[‘’]/g,"'"),start=raw.indexOf("{"),end=raw.lastIndexOf("}");if(start<0||end<start)throw new Error();raw=raw.slice(start,end+1);try{extraction=JSON.parse(raw);}catch{const repaired=raw.replace(/\\"/g,'"').replace(/\\([\[\]])/g,"$1");extraction=JSON.parse(repaired);}if(Array.isArray(extraction?.rows))extraction.rows=extraction.rows.map(row=>({...row,warnings:Array.isArray(row.warnings)?row.warnings:row.warnings?[String(row.warnings)]:[]}));if(extraction&&!Array.isArray(extraction.imageWarnings))extraction.imageWarnings=extraction.imageWarnings?[String(extraction.imageWarnings)]:[];}if(!validExtraction(extraction)||extraction.side!==$("input[name=orderSide]:checked").value)throw new Error();renderExtraction();$("#chatgptJson").closest("label").hidden=true;$("#chatgptWorkflow").hidden=true;const guide=$("#orderReviewGuide");guide.hidden=false;setTimeout(()=>guide.scrollIntoView({behavior:"smooth",block:"start"}),0);toast("表を読み込みました。各行を確認・修正してください");}catch{toast("回答の表を読み取れませんでした。ChatGPTの回答全体をもう一度コピーしてください");}}
  ...
  function setup(){
    ...
    const reviewGuide=document.createElement("p");reviewGuide.id="orderReviewGuide";reviewGuide.className="order-review-guide";reviewGuide.hidden=true;reviewGuide.innerHTML="<b>次にすること</b><br>各行の氏名・背番号・守備番号を確認し、必要な箇所を修正してください。<br>先発9名の情報が揃うと、一番下の「確認済みオーダーを確定」を押せます。";$("#extractionRows").before(reviewGuide);
