(() => {
  ...
  function updateJsonImportButtons(){[["#rosterChatgptJson","#importRosterChatgptJson"],["#chatgptJson","#importChatgptJson"]].forEach(([input,button])=>{const target=$(button);target.classList.add("json-import-action");target.disabled=!$(input).value.trim();});}
