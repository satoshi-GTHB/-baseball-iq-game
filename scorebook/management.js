(() => {
  ...
  function chatgptPrompt(){const side=$("input[name=orderSide]:checked").value;return `野球のオーダー票を読み取り、元のスコアブックアプリへ戻るまで、利用者を短い日本語で順番に案内してください。操作案内とJSONを同じ回答へ混ぜないでください。\n\nまだ写真が添付されていない場合は、次の文章だけを表示し、写真が送られるまで待ってください。\n「入力欄の＋を押し、カメラを選んでオーダー票を撮影してください。撮影後、そのまま写真のみを送信していただければ、アプリに貼り付けるための回答を返します。回答が表示されたら、その回答をそのままコピーし、元のスコアブックアプリへ戻って『② 回答を取り込む』を押してください。」\n\n写真が届いたら追加の指示を求めず、直ちに読み取ってください。先発9名に加え、記載があればベンチ入りメンバーも含め最大20名を読み取ってください。背番号はuniformNumberRaw、先発選手の守備番号（1～9）はdefensiveNumberRawへ入れてください。推測で空欄を埋めず、不明箇所は空文字にしてwarningsへ理由を書いてください。sideは必ず"${side}"にしてください。先発はbattingOrderを1～9、ベンチ入りメンバーは0、rowsは最大20名、confidenceは0～1にしてください。\n\n写真を読み取った回答では、説明・挨拶・Markdownコード枠を一切付けず、次の形の有効なJSONだけを返してください。\n{"teamNameRaw":"","side":"${side}","rows":[{"battingOrder":1,"playerNameRaw":"","uniformNumberRaw":"","defensiveNumberRaw":"","confidence":0,"warnings":[]},{"battingOrder":0,"playerNameRaw":"","uniformNumberRaw":"","defensiveNumberRaw":"","confidence":0,"warnings":[]}],"overallConfidence":0,"imageWarnings":[]}\nJSONの前後に文字を付けず、利用者へ「出力」などの追加送信も求めないでください。JSONを文字列としてエスケープせず、引用符の前にバックスラッシュ（\\）を付けないでください。`;}
  ...
  async function launchChatgptOrderReader(){const opened=window.open("https://chatgpt.com/","_blank","noopener,noreferrer"),button=$("#importChatgptJson");button.dataset.ready="true";button.classList.add("next-action");updateJsonImportButtons();try{await navigator.clipboard.writeText(chatgptPrompt());toast("ChatGPTを開きました。指示文を貼り付け、ChatGPT内のカメラで撮影してください");}catch{toast(opened?"ChatGPTを開きました。指示文のコピーを許可してください":"ChatGPTを開けませんでした");}}
  ...
  function importChatgptJson(){try{let raw=$("#chatgptJson").value.trim().replace(/[“”]/g,'"').replace(/[‘’]/g,"'"),start=raw.indexOf("{"),end=raw.lastIndexOf("}");if(start<0||end<start)throw new Error();raw=raw.slice(start,end+1);try{extraction=JSON.parse(raw);}catch{extraction=JSON.parse(raw.replace(/\\"/g,'"'));}if(Array.isArray(extraction?.rows))extraction.rows=extraction.rows.map(row=>({...row,warnings:Array.isArray(row.warnings)?row.warnings:row.warnings?[String(row.warnings)]:[]}));if(extraction&&!Array.isArray(extraction.imageWarnings))extraction.imageWarnings=extraction.imageWarnings?[String(extraction.imageWarnings)]:[];if(!validExtraction(extraction)||extraction.side!==$("input[name=orderSide]:checked").value)throw new Error();renderExtraction();$("#chatgptJson").closest("label").hidden=true;toast("回答を読み込みました。内容を確認してください");}catch{toast("回答の形式を読み取れませんでした。ChatGPTの回答全体をもう一度コピーしてください");}}
  ...
  function setup(){
    ...
    orderSteps.innerHTML="<li><b>①を押すと、指示文がコピーされてChatGPTが開きます。</b></li><li>開いたChatGPTの入力欄を長押しし、<b>「ペースト」→「送信」</b>を押します。</li><li>ChatGPTの案内どおりに撮影し、<b>写真だけを送信</b>します。</li><li>自動で返されたJSON回答をコピーしてこの画面へ戻り、<b>② 回答を取り込む</b>を押します。</li>";
