(() => {
  ...
  function openOrderHub(){
    const panel=$("#orderPanel"),state=window.ScorebookGame?.snapshot(),setup=state?.orderSetup||{topTeam:0,confirmed:false,registered:[false,false]};
    ...
    const confirm=$("#confirmOrderSetup");confirm.disabled=!setup.registered?.every(Boolean);confirm.textContent=setup.confirmed?"確定済み（変更する場合は再確定）":"2チームの設定を確定";
    $("#orderHubStatus").textContent=setup.confirmed?`${state.teams[setup.topTeam].name}が先攻、${state.teams[1-setup.topTeam].name}が後攻です`:(confirm.disabled?"両チームのオーダー登録後に確定できます":"表・裏を選んで確定してください");showPanel("orderPanel");
  }
  ...
  function setup(){
    ...
    $("#confirmOrderSetup").onclick=()=>{try{window.ScorebookGame?.setOrderSetup(+$("input[name=topTeam]:checked").value);toast("2チームと表・裏を確定しました");openOrderHub();}catch(error){toast(error.message);}};
    document.addEventListener("click",event=>{const control=event.target.closest("[data-pitch],[data-contact],[data-result],[data-hit],[data-error],[data-run-event],#droppedThirdStrike");if(control&&!window.ScorebookGame?.allowRecordingStart()){event.preventDefault();event.stopImmediatePropagation();}},true);
