(() => {
  ...
  function openOrderHub(){
    ...
    $("#orderHubTeamAName").textContent=state?.teams?.[0]?.name||"未登録";$("#orderHubTeamBName").textContent=state?.teams?.[1]?.name||"未登録";
    ...
    $("#orderHubStatus").textContent=setup.phase==="finished"?"ゲームセット済みです。新しい試合は「新規作成」から始めてください":setup.confirmed?`${state.teams[setup.topTeam].name}が先攻、${state.teams[1-setup.topTeam].name}が後攻です`:(ready?"表・裏を確認し、「プレイボール」を押してください":"両チームのオーダーを登録してください");showPanel("orderPanel");
  }
  ...
  function rosterPrompt(){return `野球チームの名簿を表へ転記してください。深い推論、説明、挨拶は不要です。\n\n資料が未添付なら、次の文だけを返して待ってください。\n「入力欄の＋からカメラで名簿を撮影するか、名簿PDFを添付し、そのまま送信してください。読取結果が出たら回答全体をコピーし、元のスコアブックアプリへ戻って『② 回答を取り込む』を押してください。」\n\n写真またはPDFを受け取ったら質問せず直ちに読み取り、次の形式だけを返してください。JSON、Markdown、コード枠、説明は禁止です。各項目は半角 | で区切り、値には | を使わないでください。\nTEAM|チーム名\nCOLUMNS|選手名|氏名かな|背番号|主な守備|確信度|警告\nROW|山田 太郎|やまだ たろう|7|遊撃手|0.90|\nROW|佐藤 次郎||12||0.70|氏名かな不明\n\n名簿に記載された選手を1名1行、最大100名まで出力してください。主な守備は投手、捕手、一塁手、二塁手、三塁手、遊撃手、左翼手、中堅手、右翼手、指名打者を「、」で区切ってください。読めない値は推測せず空欄にし、理由を警告へ短く記載してください。TEAM・COLUMNS・ROW以外の行は追加しないでください。`;}
  async function copyRosterPrompt(){const field=$("#rosterChatgptJson");field.value="";field.closest("label").hidden=true;const sessionId=crypto.randomUUID?.()||`${Date.now()}-${Math.random().toString(36).slice(2)}`;window.open(`https://chatgpt.com/?temporary-chat=true&scorebook-roster=${encodeURIComponent(sessionId)}`,"_blank","noopener,noreferrer");try{await navigator.clipboard.writeText(rosterPrompt());const button=$("#importRosterChatgptJson");button.dataset.ready="true";button.disabled=false;button.classList.add("next-action");toast("新しいChatGPTを開きました。指示文を貼り付け、カメラ撮影またはPDF添付を選んでください");}catch{field.closest("label").hidden=false;field.value=rosterPrompt();toast("指示文をコピーできませんでした。表示された指示文をコピーしてください");}}
  ...
  function parseRosterMatrix(raw){const compact=raw.replace(/^```[^\n]*\n?/i,"").replace(/```\s*$/i,"").replace(/\r?\n/g," ").replace(/\s+/g," ").trim(),teamName=compact.match(/TEAM\|([\s\S]*?)(?=\s+COLUMNS\|)/i)?.[1]?.trim()||"",list=[],pattern=/ROW\|([^|]*)\|([^|]*)\|([^|]*)\|([^|]*)\|([^|]*)\|([\s\S]*?)(?=\s+ROW\||$)/gi;for(const match of compact.matchAll(pattern)){list.push({canonicalName:match[1].trim(),nameKana:match[2].trim(),aliases:[],uniformNumber:match[3].trim(),preferredPositions:match[4].split(/[、,]/).map(value=>value.trim()).filter(Boolean),active:true,confidence:Number(match[5])||0,warnings:match[6].trim()?[match[6].trim()]:[]});if(list.length===100)break;}return teamName&&list.length?{teamName:teamName,players:list}:null;}
  function importRosterChatgptJson(){try{const raw=$("#rosterChatgptJson").value.trim();rosterExtraction=parseRosterMatrix(raw);if(!rosterExtraction){const cleaned=raw.replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/,"");rosterExtraction=JSON.parse(cleaned);}if(!validRosterExtraction(rosterExtraction))throw new Error("表形式が正しくありません");renderRosterExtraction();$("#rosterPdfWorkflow").hidden=true;toast("取り込みが完了しました。全員を確認・修正してください");}catch(error){$("#rosterChatgptJson").closest("label").hidden=false;$("#rosterChatgptJson").focus();toast(`読み込めません：${error.message}`);}}
  async function importRosterFromClipboard(){const field=$("#rosterChatgptJson");if(field.value.trim()){importRosterChatgptJson();return;}try{const text=(await navigator.clipboard.readText()).trim();if(!text)throw new Error();field.value=text;importRosterChatgptJson();}catch{field.closest("label").hidden=false;field.focus();toast("自動取得が許可されませんでした。ここへ回答を貼り付けると読み込めます");}}
  ...
  function setup(){
    ...
    const hub=document.createElement("div");hub.id="orderHub";hub.className="order-hub";hub.innerHTML=`<h2>試合前オーダー</h2><p class="order-hub-lead">2チームのオーダーを登録し、先攻（表）を選択してください。</p><div class="order-team-cards"><section id="orderHubTeamA"><b>チームA</b><strong id="orderHubTeamAName">未登録</strong><small id="orderHubTeamAStatus">未登録</small><button type="button" data-edit-order="own">入力・確認する</button></section><section id="orderHubTeamB"><b>チームB</b><strong id="orderHubTeamBName">未登録</strong><small id="orderHubTeamBStatus">未登録</small><button type="button" data-edit-order="opponent">入力・確認する</button></section></div><fieldset class="top-team-choice"><legend>表（先攻）のチーム</legend><label><input id="topTeamA" type="radio" name="topTeam" value="0"> チームA</label><label><input id="topTeamB" type="radio" name="topTeam" value="1"> チームB</label></fieldset><button type="button" id="confirmOrderSetup" hidden>プレイボール</button><p id="orderHubStatus" role="status"></p><p class="order-hub-note">「プレイボール」を押すまではスコアを入力できません。</p><button type="button" id="newOrderGame">新規作成</button><div id="orderResetDialog" class="order-reset-dialog" hidden role="dialog" aria-modal="true" aria-labelledby="orderResetTitle"><div class="order-reset-dialog-card"><h3 id="orderResetTitle">新規作成</h3><p>両チームのメンバー表が削除されます。新規作成してよいですか？</p><div><button type="button" id="cancelOrderReset">キャンセル</button><button type="button" id="confirmOrderReset">OK</button></div></div></div></div>`;
    ...
    $("#exportRoster").onclick=downloadRoster; $("#importRoster").onchange=e=>importRoster(e.target.files[0]);
