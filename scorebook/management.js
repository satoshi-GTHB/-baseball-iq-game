(() => {
  ...
  function importChatgptJson(){try{let raw=$("#chatgptJson").value.trim().replace(/[“”]/g,'"').replace(/[‘’]/g,"'"),start=raw.indexOf("{"),end=raw.lastIndexOf("}");if(start<0||end<start)throw new Error();raw=raw.slice(start,end+1);try{extraction=JSON.parse(raw);}catch{const repaired=raw.replace(/\\"/g,'"').replace(/\\([\[\]])/g,"$1");extraction=JSON.parse(repaired);}if(Array.isArray(extraction?.rows))extraction.rows=extraction.rows.map(row=>({...row,warnings:Array.isArray(row.warnings)?row.warnings:row.warnings?[String(row.warnings)]:[]}));if(extraction&&!Array.isArray(extraction.imageWarnings))extraction.imageWarnings=extraction.imageWarnings?[String(extraction.imageWarnings)]:[];if(!validExtraction(extraction)||extraction.side!==$("input[name=orderSide]:checked").value)throw new Error();renderExtraction();$("#chatgptJson").closest("label").hidden=true;toast("回答を読み込みました。内容を確認してください");}catch{toast("回答の形式を読み取れませんでした。ChatGPTの回答全体をもう一度コピーしてください");}}
  ...
  function setup(){
    ...
    orderSteps.innerHTML="<li><b>①を押すと、指示文がコピーされ、新しい一時チャットが開きます。</b></li><li>開いたChatGPTの入力欄を長押しし、<b>「ペースト」→「送信」</b>を押します。</li><li>ChatGPTの案内どおりに撮影し、<b>写真だけを送信</b>します。</li><li>自動で返されたJSON回答をコピーしてこの画面へ戻り、<b>② 回答を取り込む</b>を押します。</li>";
