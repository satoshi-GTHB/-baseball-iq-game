(() => {
  ...
  function importChatgptJson(){try{let raw=$("#chatgptJson").value.trim().replace(/[“”]/g,'"').replace(/[‘’]/g,"'"),start=raw.indexOf("{"),end=raw.lastIndexOf("}");if(start<0||end<start)throw new Error("JSONが見つかりません");raw=raw.slice(start,end+1);extraction=JSON.parse(raw);if(Array.isArray(extraction?.rows))extraction.rows=extraction.rows.map(row=>({...row,warnings:Array.isArray(row.warnings)?row.warnings:row.warnings?[String(row.warnings)]:[]}));if(extraction&&!Array.isArray(extraction.imageWarnings))extraction.imageWarnings=extraction.imageWarnings?[String(extraction.imageWarnings)]:[];if(!validExtraction(extraction)||extraction.side!==$("input[name=orderSide]:checked").value)throw new Error("JSON形式またはチーム区分が正しくありません");renderExtraction();$("#chatgptJson").closest("label").hidden=true;toast("回答を読み込みました。内容を確認してください");}catch(error){toast(`読み込めません：${error.message}`);}}
